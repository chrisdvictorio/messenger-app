...
const FindUser = ({ handleRightComponentClick, handleSelectProfileClick }) => {
  ...
  return (
    <div className="flex flex-col min-w-[30rem] p-3 border-l-2 rounded-r-2xl border-black space-y-4 bg-[#161A20]">
      <div className="flex flex-col">
        ...
        <p>
          {" "}
          {allUsers?.length} Account{allUsers?.length > 1 ? "s" : ""} Created
        </p>
      </div>
      <div className="relative flex items-center">
        <IoIosSearch className="absolute left-3 size-5 text-[#65676B]" />
        <label htmlFor="findUser" className="sr-only"></label>
        <input
          id="findUser"
          type="text"
